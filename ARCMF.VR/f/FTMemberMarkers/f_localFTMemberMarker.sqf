...
while {alive _unit && (_unit in f_var_HandlerGroup)} do {
    ...
    if (alive _unit && {_unit in units group player}) then {
        _alpha = linearConversion [0, (getNumber (missionConfigFile >> "CfgARCMF" >> "markers" >> ((side (group _unit)) call ARC_fnc_getFactionFromSide) >> "fireteamMemberFadeDistance")), (_unit distance player), 1, 0, true];
