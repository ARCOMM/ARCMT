...
_rifleClass = [_faction, "rifles", _unit] call ARC_fnc_pickItemFromAIGear;
...
_uniformClass = [_faction, "uniforms", _unit] call ARC_fnc_pickItemFromAIGear;
...
_vestClass = [_faction, "vests", _unit] call ARC_fnc_pickItemFromAIGear;
...
_backpackClass = [_faction, "backpacks", _unit] call ARC_fnc_pickItemFromAIGear;
...
_headgearClass = [_faction, "headgear", _unit] call ARC_fnc_pickItemFromAIGear;
if (count _headgearClass > 0) then {
	removeHeadgear _unit;
	_unit addHeadgear (_headgearClass select 0);
};

_goggleClass = [_faction, "goggles", _unit] call ARC_fnc_pickItemFromAIGear;
if (count _goggleClass > 0) then {
	removeGoggles _unit;
	_unit addGoggles (_goggleClass select 0);
};

_faceClass = [_faction, "faces", _unit] call ARC_fnc_pickItemFromAIGear;
if (count _faceClass > 0) then {
	_unit setFace (_faceClass select 0);
};

_voiceClass = [_faction, "voices", _unit] call ARC_fnc_pickItemFromAIGear;
...
_launcherClass = [_faction, "launchers", _unit] call ARC_fnc_pickItemFromAIGear;
