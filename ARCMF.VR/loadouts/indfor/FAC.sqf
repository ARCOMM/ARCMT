this forceAddUniform "CUP_U_B_BAF_MTP_S3_RolledUp";
for "_i" from 1 to 5 do {this addItemToUniform "ACE_fieldDressing";};
this addItemToUniform "ACE_CableTie";
this addItemToUniform "ACE_EarPlugs";
this addItemToUniform "ACE_morphine";
this addItemToUniform "ACE_tourniquet";
this addVest "CUP_V_BAF_Osprey_Mk4_MTP_Grenadier";
for "_i" from 1 to 2 do {this addItemToVest "CUP_HandGrenade_L109A1_HE";};
this addItemToVest "SmokeShell";
for "_i" from 1 to 4 do {this addItemToVest "CUP_30Rnd_556x45_Stanag";};
for "_i" from 1 to 5 do {this addItemToVest "30Rnd_556x45_Stanag_Tracer_Red";};
for "_i" from 1 to 6 do {this addItemToVest "CUP_1Rnd_HE_M203";};
for "_i" from 1 to 2 do {this addItemToVest "CUP_1Rnd_StarCluster_Red_M203";};
for "_i" from 1 to 2 do {this addItemToVest "CUP_1Rnd_StarCluster_Green_M203";};
this addBackpack "CUP_B_USPack_Coyote";
for "_i" from 1 to 4 do {this addItemToBackpack "SmokeShellGreen";};
for "_i" from 1 to 4 do {this addItemToBackpack "SmokeShellRed";};
for "_i" from 1 to 2 do {this addItemToBackpack "SmokeShell";};
for "_i" from 1 to 3 do {this addItemToBackpack "CUP_FlareRed_M203";};
for "_i" from 1 to 3 do {this addItemToBackpack "CUP_FlareGreen_M203";};
for "_i" from 1 to 3 do {this addItemToBackpack "CUP_1Rnd_SmokeGreen_M203";};
for "_i" from 1 to 3 do {this addItemToBackpack "CUP_1Rnd_SmokeRed_M203";};
for "_i" from 1 to 3 do {this addItemToBackpack "SmokeShellBlue";};
this addHeadgear "CUP_H_BAF_Helmet_3_MTP";
this addWeapon "CUP_arifle_L85A2_GL";
this addWeapon "CUP_LRTV";
this linkItem "ItemMap";
this linkItem "ItemCompass";
this linkItem "ItemWatch";
this linkItem "ItemGPS";
